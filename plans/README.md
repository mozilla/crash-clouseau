...
| `02-llm-abstraction-tiering.md` | LLM abstraction & model tiering | Single `llm_call(role, ...)` seam; per-role model/effort config; usage accounting | M | `anthropic` SDK, all 4 Claude models, `agent.llm.<role>` config, `tests/test_llm.py` runner |
...
| `12-product-wiring-ui-needinfo.md` | Product wiring: UI evidence panel + needinfo draft | Render verdict/evidence panel; surface manual needinfo target | M | `agent.ui.*`/`agent.confidence.*` config, `Verdict.get_by_uuid`, searchfox permalinks |
| `13-eval-harness.md` | Evaluation harness | Offline re-run over historical corpus; precision/abstain metrics | M | Anthropic Batch API, libmozdata RawRevision, BMO `clouseau`-alias query |
...
- `#02` LLM abstraction & tiering — the `llm_call(role, ...)` seam every model-calling role uses.
...
- `anthropic` — Claude SDK; floor unverified, pin to the version that installs on
  Heroku Python 3.14 and exposes `messages.parse` / `parsed_output` /
  `output_config.effort` / server-side fallbacks (candidates seen across sub-plans:
  `>=0.40`, `>=0.69`, `>=0.109.2` — **resolve at implement time via `pip show`**).
- `pydantic` (`>=2.7`) — strict structured-output models for dossier/verdict/citations.

(No other new Python packages. `libmozdata>=0.2.12`, `parsepatch`, `requests`,
`rq`, `redis`, `sqlalchemy`, `flask*`, `jinja2` are already present and reused.
**`#14` patch extraction adds no new deps** — it parses diff hunks with the existing
`parsepatch`/`RawRevision`/`requests`.)
...
- Batch API (`client.messages.batches.create`, 50% cheaper, async ≤24h) — offline
  eval re-runs only (#13), never live per-crash.
- Prompt caching (`cache_control={"type":"ephemeral"}`) on the shared dossier/diff
  prefix across senior calls (#02).
...
- `crashclouseau/report_bug.py` + `templates/bug.txt` — needinfo/bug rendering;
  `enter_bug` cannot prefill needinfo, so #12 surfaces the target for manual entry.
- `crashclouseau/html.py`, `templates/`, `static/clouseau.js` — UI panel (#12).
- New module namespace `crashclouseau/agent/` (`schema.py`/`dossier.py`, `llm.py`,
  `searchfox.py`, per-role modules) — created by the foundations and roles.

### New config keys (in `config/global.json`, single `agent` block — must NOT diverge)
- `agent.llm.<role>` → `{model, effort?, max_tokens?, provider?}` for each of the 6
  roles (#02, #05–#10).
- `agent.patch_scout.{max_candidates, neighborhood_file_cap, diff_byte_cap, model, enabled}` (#07).
- `agent.{per_role_timeout, budget_queries, enabled, agent_version}` (#06, #11).
- `agent.ui.{show_abstain, high_confidence_label}` and
  `agent.confidence.needinfo_min` (#12).
- Readers: `get_agent()` / `get_agent_llm(role)` / `get_patch_scout_cfg()` — names
...
- New `UUID` agent-state column (`UUID.set_agent_state`) for orchestration idempotency (#11).
...
- **libmozdata shape unverified.** Not importable in this checkout; SuperSearch param
  ...
  #05, #08, #13).
- **anthropic SDK feature presence.** `messages.parse` / `parsed_output` /
  `output_config.effort` / server-side `fallbacks` / discriminated-union `anyOf` under
  strict outputs must each be confirmed by a live one-shot call on the version pip
  resolves for Heroku Python 3.14 (#02, #03, #10).
...
- **Unowned schema seams.** The `crashclouseau/agent/` modules (`schema.py`/
  `dossier.py`, `llm.py`, `searchfox.py`) and the exact dossier field names are
...
- **Wiring not enforceable in-unit.** Per-role timeouts live in the role/adapter
  modules (RQ `job_timeout` only bounds the whole job); the actual enqueue site
  (`put_report` vs `analyze_one_report`) and a Procfile change to run a worker on the
  `agent` queue are ops follow-ups — without the Procfile change jobs enqueue but
  never run (#02, #11).
...
- **Product policy / sign-off (human decisions, not bugs).** Whether a backed-out
  patch is ever kept (#09), and product-owner acceptance that needinfo targets are
  copied manually (no Bugzilla auto-needinfo) (#12).
